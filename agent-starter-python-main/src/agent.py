...
# Wrapper pour éviter le TypeError sur supports_language
class SafeMultilingualModel(MultilingualModel):
    async def supports_language(self, lang):
        # Appelle la méthode originale mais retourne bool sans await
        return super().supports_language(lang)
...
async def entrypoint(ctx: JobContext):
    ...
    # Connexion d'abord pour éviter les warnings
    await ctx.connect()
    ...
    await session.start(
        ...
        room_input_options=RoomInputOptions(
            ...
            close_on_disconnect=False,  # Ne ferme pas la session automatiquement
        ),
    )
